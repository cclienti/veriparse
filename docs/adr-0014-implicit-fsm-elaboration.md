...
### 6.3 Interface ports — kept as they stand, their members ordinary signals

**Rule.** A module port of interface type is **passed through**: nothing
inlines it, flattens it, or rewrites its type, and the emitted module keeps
the port exactly as written (`bus_if.dev bus`). Its members are signals of
the machine like any other — read in guards and actions, committed with
`<=`, passed as task actuals — reached by their whole path (`bus.ack`),
which is what IEEE §25.3 makes them: variables of the interface instance,
visible to the module through the port.

Two consequences follow, and both are checks rather than conventions:

- **A hierarchical name keys by its path, never by its leaf.** `bus.ack`
  and a local `ack` are different signals and must never share an entry in
...
- **A hierarchical target must be a member of an interface port of this
  ...
  restricted: a machine may read what it can see.
...
entire boundary is an interface, with a member read in a guard, committed
from a task body, and carried as a task input actual.

Virtual interfaces, modport direction enforcement, and *calling* a
subroutine through an interface are not part of this: see §15. A task
declared inside an interface parses and is kept — what no grammar of ours
accepts is the hierarchical call that would reach it (`bus.ping()`), the
same gap that stops `u.t()` on a module instance.
...
**The rolled loops lower in their own pass, `FsmLoopLowering`** — a
stage of the structural pre-lowering, which runs `FsmAlphaRename`
(shadowed locals uniquify, §6.1), then `FsmTaskInliner`, then this
pass, before the walk. A rolled `repeat (count)` becomes a countdown while
...
| a cut-spanning task local carrying unpacked dimensions or an initializer | an array register is not hoisted yet, and a static local initializes once where the machine's reset re-runs — both would otherwise lose meaning in silence (§7.4) | ADR §7.4, IEEE §6.21 |
| an output formal assigned on only some paths | the copy-out reads it on every path; the all-paths write is the property (§7.4) | ADR §7.4, IEEE §13.3 |
...
| a `=` temporary shadowing a module-level declaration or an enclosing temporary | `FsmAlphaRename` uniquifies it before anything binds by name — the shadowed forms compile as written; the walk's shadow check survives as an internal guard | ADR §6 |
...
| Virtual interfaces, calling a subroutine **through** an interface (§25.5), and modport **direction** enforcement | interface **ports** are supported and passed through (§6.3): members read, committed, and carried through task formals, the port kept as written. A `virtual` interface port is refused by the parser. A task or function *declared* inside an interface parses and survives; what does not parse is any **hierarchical call** — `bus.ping()`, `bus.gv()`, and equally `u.t()` on a module instance — because the grammar builds `task_call`/`function_call` from a bare identifier only (the scope form `pkg::t()` is the one qualified call it accepts, which is how package tasks reach a machine, §7.4). So this is a general parser gap, not an interface rule, and it stops the shape before this pass. Writing a member a modport declares `input` is passed through, and the consumer refuses it with its own message (measured: Verilator, *Attempt to drive input-only modport*) | direction enforcement is a §9 row the day the modport is resolved here — it needs the interface definition in hand, which is `InterfaceElaboration`'s (ADR-0008) knowledge, not this pass's. Virtual interfaces need a dynamic-binding model and belong with that ADR |
