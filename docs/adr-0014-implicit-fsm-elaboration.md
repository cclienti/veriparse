...
Virtual interfaces and modport direction enforcement are not part of
this: see §15. *Calling* a subroutine through an interface port was the
same §15 row until ADR-0015 closed it: `bus.ping()` now parses and
resolves by splicing (`HierCallResolution`), reducing to exactly the
shapes this section supports — the module-instance form `u.t()` stays
refused there.
...
| Virtual interfaces and modport **direction** enforcement (calling a subroutine **through** an interface port, §25.7, sat in this row until ADR-0015 closed it) | interface **ports** are supported and passed through (§6.3): members read, committed, and carried through task formals, the port kept as written. A `virtual` interface port is refused by the parser. `bus.ping()` now parses and resolves by splicing (`HierCallResolution`, ADR-0015) before this pass — its own §8 keeps `u.t()` on a module instance, local interface instances, and body closure parked. Writing a member a modport declares `input` is passed through, and the consumer refuses it with its own message (measured: Verilator, *Attempt to drive input-only modport*) | direction enforcement is a §9 row the day the modport is resolved here — it needs the interface definition in hand, which is `InterfaceElaboration`'s (ADR-0008) knowledge, not this pass's. Virtual interfaces need a dynamic-binding model and belong with that ADR |
