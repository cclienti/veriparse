...
{ % -set properties_user_enum_full = merge_dict(ndict['properties_user_enum'],
                                                inherited_flat['properties_user_enum']) % }
    ...
    if(node.IsMap()) {
        if(node["filename"]) {
            if(node["filename"].IsScalar()) {
                if(!result)
                    result = std::make_shared<AST::{{ntype}}>();
        ...
        if(node["{{ pname }}"]) {
            if(node["{{ pname }}"].IsScalar()) {
                ...
                result->set_{{pname}}(
                    node["{{ pname }}"]
                        .as<{ % -if ptype in ndict['properties_user_enum'] - % } AST::{
                            {
                                ntype
                            }
                        } ::{ % -else - % } {
                            % -if ptype in inherited_flat['properties_user_enum'] - %
                        } AST::{ % -endif - % } { % -endif - % } {{ptype}}>());
            }
        }
        {
            % -endfor %
            ...
            if(node_{{cname}}.IsSequence()) {
                ...
                for(auto it = node_{{cname}}.begin(); it != node_{{cname}}.end(); ++it) {
                    ...
                    if(child) {
                        ...
                        child_cast = AST::cast_to<AST::{{ctype_under}}>(child);
                        {
                            {
                                cname
                            }
                        }
                        _list->push_back(child_cast);
                        {
                            % -else %
                        }
                        {
