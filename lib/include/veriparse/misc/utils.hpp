...
template <typename Container> struct VIteratorDrop
{
    ...
    explicit VIteratorDrop(Container &&container, bool drop_first, bool drop_last)
        : m_container(std::forward<Container>(container)), m_drop_first(drop_first),
...
template <typename Container> inline auto viter_drop_last(Container &&container)
{
    return VIteratorDrop<Container>(std::forward<Container>(container), false, true);
}

template <typename Container> inline auto viter_drop_first(Container &&container)
{
    return VIteratorDrop<Container>(std::forward<Container>(container), true, false);
}

template <typename Container> inline auto viter_drop_both(Container &&container)
{
