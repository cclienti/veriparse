...
template <class TDest> struct CastTo
{
     ...
     */
    template <class TSrcPtr> static Ptr ptr(TSrcPtr node)
    {
        constexpr NodeType dest_node_type = get_node_type<TDest>();

        if(node) {
            if((node->is_node_type(dest_node_type)) || (node->is_node_category(dest_node_type))) {
                return std::static_pointer_cast<TDest>(node);
            } else {
                LOG_ERROR << "node type \"" << node->get_node_type() << "\" cannot be casted to \""
                          << dest_node_type << "\"";
                return nullptr;
            }
        } else {
     ...
     * type.
     */
    template <class TSrcListPtr> static ListPtr list_ptr(TSrcListPtr nodes)
    {
 ...
 */
template <class TDest, class TSrcPtr> inline typename CastTo<TDest>::Ptr cast_to(TSrcPtr node)
{
 ...
 * Convenient function to cast to any node pointer type
 */
template <class TDest> inline typename CastTo<TDest>::Ptr cast_to(AST::Node::Ptr node)
{
    return CastTo<TDest>::ptr(node);
}
 ...
 */
template <class TSrcPtr> inline Node::Ptr to_node(TSrcPtr node)
{
 ...
 */
template <class TSrcListPtr> inline Node::ListPtr to_node_list(TSrcListPtr nodes)
{
