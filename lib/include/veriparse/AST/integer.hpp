#ifndef VERIPARSE_AST_INTEGER_HPP
#define VERIPARSE_AST_INTEGER_HPP
...
namespace Veriparse {
	namespace AST {
		...
		public:
			 ...
			 */
			Integer(const std::string &filename="", uint32_t line=0);
			
			/**
			 * Constructor, m_node_type is set to NodeType::Integer.
			 */
			Integer(const Length::ListPtr lengths, const Rvalue::Ptr right, const std::string &name, const std::string &filename="", uint32_t line=0);
			...
			/**
			 * Return true if the Integer nodes are the same, do not check children.
			 */
			virtual bool operator==(const Integer &rhs) const;
