#ifndef VERIPARSE_AST_LENGTH_HPP
#define VERIPARSE_AST_LENGTH_HPP
...
public:
	 ...
	 */
	Length(const std::string &filename="", uint32_t line=0);
	
	/**
	 * Constructor, m_node_type is set to NodeType::Length.
	 */
	Length(const Node::Ptr msb, const Node::Ptr lsb, const std::string &filename="", uint32_t line=0);
	...
	/**
	 * Return true if the Length nodes are the same, do not check children.
	 */
