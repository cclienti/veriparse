...
#ifndef VERIPARSE_AST_ENUMITEM_HPP
...
{
public:
    using Ptr = typename NodePointers<EnumItem>::Ptr;
    using List = typename NodePointers<EnumItem>::List;
    using ListPtr = typename NodePointers<EnumItem>::ListPtr;
    using Node::operator=;
    using Node::operator==;
    using Node::operator!=;
     ...
     */
    EnumItem(const std::string &filename = "", uint32_t line = 0);
     ...
     */
    virtual EnumItem &operator=(const EnumItem &rhs);
    ...
    /**
     * Return true if the EnumItem nodes are the same, do not check children.
     */
    virtual bool operator==(const EnumItem &rhs) const;
