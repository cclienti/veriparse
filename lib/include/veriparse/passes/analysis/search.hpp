...
template <AST::NodeType... blacklists> class Search
{
public:
    Search() = delete;
...
protected:
                              ...
                              typename T::ListPtr list, int64_t max_occur_depth = -1)
    {
        if(std::abs(max_occur_depth) > 0) {
            ...
            if(blacklisted_nodetype<blacklists...>(current_nt)) {
                return;
            }
...
private:
    static bool blacklisted_nodetype(AST::NodeType current) { return false; }

    template <AST::NodeType nt0> static bool blacklisted_nodetype(AST::NodeType current)
    {
        return (current == nt0);
    }
