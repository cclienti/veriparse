...
inline AST::FloatConst::Ptr intconstn_to_floatconst(AST::IntConstN::Ptr x)
{
    ...
    if(x) {
        res = std::make_shared<AST::FloatConst>(x->get_value().convert_to<double>(),
                                                x->get_filename(), x->get_line());
    }
    return res;
}
...
{
    if(x && y) {
        if((x->is_node_type(AST::NodeType::IntConstN)) &&
           (y->is_node_type(AST::NodeType::IntConstN))) {
    ...
    if(x && y) {
        if((x->is_node_type(AST::NodeType::IntConstN)) &&
           (y->is_node_type(AST::NodeType::IntConstN))) {
            BINARY_OP<AST::IntConstN::Ptr> op;
            return op(AST::cast_to<AST::IntConstN>(x), AST::cast_to<AST::IntConstN>(y));
        } else if((x->is_node_type(AST::NodeType::FloatConst)) &&
                  (y->is_node_type(AST::NodeType::IntConstN))) {
            ...
            return op(xc, yc);
        } else if((x->is_node_type(AST::NodeType::IntConstN)) &&
                  (y->is_node_type(AST::NodeType::FloatConst))) {
            BINARY_OP<AST::FloatConst::Ptr> op;
...
AST::Node::Ptr expression_operator_ternary(const AST::Node::Ptr x, const AST::Node::Ptr y,
                                           const AST::Node::Ptr z)
{
    if(x && y && z) {
