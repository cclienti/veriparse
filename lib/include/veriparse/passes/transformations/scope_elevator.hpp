...
public:
    ...
    using GlobalReplaceMap = std::map<std::string, std::pair<ScopeStack, std::string>>;
...
private:
    ...
    /// True while walking a process marked (* veriparse_fsm *): every block
    /// is kept in place. Named ones are renamed as usual — their labels name
