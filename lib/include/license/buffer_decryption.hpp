                                 ...
                                 std::string const &input_buffer, std::string &output_buffer)
{
	...
	uint32_t rsa_encrypted_aes_key_size = rsa_encrypted_aes_key_buffer.size()/2;

	uint8_t *rsa_encrypted_aes_key = new uint8_t[rsa_encrypted_aes_key_size];
	for(uint32_t i=0; i<rsa_encrypted_aes_key_buffer.size(); i+=2) {
	...
	uint8_t *aes_key = new uint8_t[RSA_size(rsa_public_key)];
	...
	uint8_t aes_decrypt_input[16];
	uint8_t aes_decrypt_output[16+1];

	output_buffer.clear();

	for(uint32_t block=0; block<input_buffer.size(); block+=32) {
