...
#ifdef FULL_DEBUG
// log all messages
static const auto severity_level = boost::log::trivial::trace;
#elif DEBUG
// log debug but not trace
static const auto severity_level = boost::log::trivial::debug;
#else
// log all messages except debug and trace
static const auto severity_level = boost::log::trivial::info;
#endif
...
void add_stdout_sink(void)
{
    ...
    // Set debug level
    ostream_sink->set_filter(severity >= severity_level);
...
void add_text_sink(std::string const &filename)
{
    ...
    // Set debug level
    file_sink->set_filter(severity >= severity_level);
