...
Uminus::Uminus(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Uminus::operator!=(const Uminus &rhs) const { return !(operator==(rhs)); }

bool Uminus::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Uminus::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Uminus>(new_node)->set_right(get_right()->clone());
    }
}
