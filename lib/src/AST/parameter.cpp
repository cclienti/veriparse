...
Parameter::Parameter(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Parameter);
    set_node_categories({NodeType::Node});
}

Parameter::Parameter(const Node::Ptr value, const Width::ListPtr widths, const std::string &name,
                     const bool &sign, const TypeEnum &type, const std::string &filename,
                     uint32_t line)
    : Node(filename, line), m_value(value), m_widths(widths), m_name(name), m_sign(sign),
      m_type(type)
{
    set_node_type(NodeType::Parameter);
    set_node_categories({NodeType::Node});
...
bool Parameter::operator!=(const Parameter &rhs) const { return !(operator==(rhs)); }

bool Parameter::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Parameter::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_value()) {
        cast_to<Parameter>(new_node)->set_value(get_value()->clone());
    }
    cast_to<Parameter>(new_node)->set_widths(Width::clone_list(get_widths()));
}
...
{
    switch(p) {
    case Parameter::TypeEnum::INTEGER:
        os << "INTEGER";
        break;
    case Parameter::TypeEnum::REAL:
        os << "REAL";
        break;
    case Parameter::TypeEnum::LOGIC:
        os << "LOGIC";
        break;
    case Parameter::TypeEnum::INT:
        os << "INT";
        break;
    case Parameter::TypeEnum::BIT:
        os << "BIT";
        break;
    case Parameter::TypeEnum::BYTE:
        os << "BYTE";
        break;
    case Parameter::TypeEnum::SHORTINT:
        os << "SHORTINT";
        break;
    case Parameter::TypeEnum::LONGINT:
        os << "LONGINT";
        break;
    case Parameter::TypeEnum::NONE:
        os << "NONE";
        break;
    default:
        break;
