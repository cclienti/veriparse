...
Uand::Uand(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Uand::operator!=(const Uand &rhs) const { return !(operator==(rhs)); }

bool Uand::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Uand::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Uand>(new_node)->set_right(get_right()->clone());
    }
}
