...
PartselectIndexed::PartselectIndexed(const Node::Ptr index, const Node::Ptr size,
                                     const Node::Ptr var, const std::string &filename,
                                     uint32_t line)
...
bool PartselectIndexed::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool PartselectIndexed::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_index()) {
        cast_to<PartselectIndexed>(new_node)->set_index(get_index()->clone());
    }
    if(get_size()) {
        cast_to<PartselectIndexed>(new_node)->set_size(get_size()->clone());
    }
    if(get_var()) {
        cast_to<PartselectIndexed>(new_node)->set_var(get_var()->clone());
    }
}
