...
GenerateStatement::GenerateStatement(const Node::ListPtr items, const std::string &filename,
                                     uint32_t line)
...
bool GenerateStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool GenerateStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const Node::Ptr &lnode : *get_items()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "GenerateStatement::replace matches multiple times "
                                       "(list(Node)::items)";
                    }
                    if(new_node) {
...
{
    cast_to<GenerateStatement>(new_node)->set_items(Node::clone_list(get_items()));
}
