...
EventStatement::EventStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::EventStatement);
    set_node_categories({NodeType::Node});
}

EventStatement::EventStatement(const Senslist::Ptr senslist, const Node::Ptr statement,
...
bool EventStatement::operator!=(const EventStatement &rhs) const { return !(operator==(rhs)); }

bool EventStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool EventStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_senslist()) {
        cast_to<EventStatement>(new_node)->set_senslist(cast_to<Senslist>(get_senslist()->clone()));
    }
    if(get_statement()) {
        cast_to<EventStatement>(new_node)->set_statement(get_statement()->clone());
    }
}
