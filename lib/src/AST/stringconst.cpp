...
StringConst::StringConst(const std::string &filename, uint32_t line) : Constant(filename, line)
{
...
bool StringConst::operator!=(const StringConst &rhs) const { return !(operator==(rhs)); }

bool StringConst::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool StringConst::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
