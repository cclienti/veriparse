...
ImplicitType::ImplicitType(const std::string &filename, uint32_t line) : Variable(filename, line)
{
    set_node_type(NodeType::ImplicitType);
    set_node_categories({NodeType::Variable, NodeType::VariableBase, NodeType::Node});
}

ImplicitType::ImplicitType(const Width::ListPtr widths, const Length::ListPtr lengths,
...
bool ImplicitType::replace(Node::Ptr node, Node::Ptr new_node)
{
            ...
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING
                            << *this << ", "
                            << "ImplicitType::replace matches multiple times (list(Width)::widths)";
                    }
                    if(new_node) {
    ...
    if(get_right()) {
        if(get_right() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "ImplicitType::replace matches multiple times (Rvalue::right)";
            }
...
{
    cast_to<ImplicitType>(new_node)->set_widths(Width::clone_list(get_widths()));
    cast_to<ImplicitType>(new_node)->set_lengths(Length::clone_list(get_lengths()));
    if(get_right()) {
        cast_to<ImplicitType>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
}
