...
Lvalue::Lvalue(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Lvalue::operator!=(const Lvalue &rhs) const { return !(operator==(rhs)); }

bool Lvalue::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Lvalue::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_var()) {
        cast_to<Lvalue>(new_node)->set_var(get_var()->clone());
    }
}
