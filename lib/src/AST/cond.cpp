...
Cond::Cond(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::Cond);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

Cond::Cond(const Node::Ptr cond, const Node::Ptr left, const Node::Ptr right,
...
bool Cond::operator!=(const Cond &rhs) const { return !(operator==(rhs)); }

bool Cond::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Cond::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_cond()) {
        cast_to<Cond>(new_node)->set_cond(get_cond()->clone());
    }
    if(get_left()) {
        cast_to<Cond>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Cond>(new_node)->set_right(get_right()->clone());
    }
}
