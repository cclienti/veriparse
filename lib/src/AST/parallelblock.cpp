...
ParallelBlock::ParallelBlock(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::ParallelBlock);
    set_node_categories({NodeType::Node});
}

ParallelBlock::ParallelBlock(const Node::ListPtr statements, const std::string &scope,
...
bool ParallelBlock::operator!=(const ParallelBlock &rhs) const { return !(operator==(rhs)); }

bool ParallelBlock::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool ParallelBlock::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const Node::Ptr &lnode : *get_statements()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "ParallelBlock::replace matches multiple times "
...
{
    cast_to<ParallelBlock>(new_node)->set_statements(Node::clone_list(get_statements()));
}
