...
NonblockingSubstitution::NonblockingSubstitution(const Lvalue::Ptr left, const Rvalue::Ptr right,
...
bool NonblockingSubstitution::operator!=(const Node &rhs) const { return !(operator==(rhs)); }
...
bool NonblockingSubstitution::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_ldelay()) {
        if(get_ldelay() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "NonblockingSubstitution::replace matches multiple times "
...
{
    if(get_left()) {
        cast_to<NonblockingSubstitution>(new_node)->set_left(cast_to<Lvalue>(get_left()->clone()));
    }
    if(get_right()) {
