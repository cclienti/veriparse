...
Output::Output(const std::string &filename, uint32_t line) : IODir(filename, line)
{
    set_node_type(NodeType::Output);
    set_node_categories({NodeType::IODir, NodeType::Node});
}

Output::Output(const Width::ListPtr widths, const std::string &name, const bool &sign,
...
bool Output::operator!=(const Output &rhs) const { return !(operator==(rhs)); }

bool Output::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Output::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Output>(new_node)->set_widths(Width::clone_list(get_widths()));
}
