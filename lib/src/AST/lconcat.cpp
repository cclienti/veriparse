...
Lconcat::Lconcat(const std::string &filename, uint32_t line) : Concat(filename, line)
{
...
bool Lconcat::operator!=(const Lconcat &rhs) const { return !(operator==(rhs)); }

bool Lconcat::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Lconcat::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool Lconcat::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_list()) {
        ...
        for(const Node::Ptr &lnode : *get_list()) {
            if(lnode) {
                if(lnode != node) {
...
{
    cast_to<Lconcat>(new_node)->set_list(Node::clone_list(get_list()));
}
