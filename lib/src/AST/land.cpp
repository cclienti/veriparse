...
Land::Land(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Land::operator!=(const Land &rhs) const { return !(operator==(rhs)); }

bool Land::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Land::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Land>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Land>(new_node)->set_right(get_right()->clone());
    }
}
