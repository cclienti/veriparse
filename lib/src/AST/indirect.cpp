...
Indirect::Indirect(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Indirect::operator!=(const Indirect &rhs) const { return !(operator==(rhs)); }

bool Indirect::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Indirect::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_var()) {
        cast_to<Indirect>(new_node)->set_var(get_var()->clone());
    }
}
