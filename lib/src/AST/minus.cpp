...
Minus::Minus(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::Minus);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

Minus::Minus(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
             uint32_t line)
...
bool Minus::operator!=(const Minus &rhs) const { return !(operator==(rhs)); }

bool Minus::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Minus::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Minus>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Minus>(new_node)->set_right(get_right()->clone());
    }
}
