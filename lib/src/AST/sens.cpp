...
Sens::Sens(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Sens::operator!=(const Sens &rhs) const { return !(operator==(rhs)); }

bool Sens::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Sens::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    switch(p) {
    case Sens::TypeEnum::ALL:
        os << "ALL";
        break;
    case Sens::TypeEnum::POSEDGE:
        os << "POSEDGE";
        break;
    case Sens::TypeEnum::NEGEDGE:
        os << "NEGEDGE";
        break;
    case Sens::TypeEnum::NONE:
        os << "NONE";
        break;
    default:
        break;
