...
PartselectMinusIndexed::PartselectMinusIndexed(const Node::Ptr index, const Node::Ptr size,
...
bool PartselectMinusIndexed::operator!=(const Node &rhs) const { return !(operator==(rhs)); }
...
{
    if(get_index()) {
        cast_to<PartselectMinusIndexed>(new_node)->set_index(get_index()->clone());
    }
    if(get_size()) {
        cast_to<PartselectMinusIndexed>(new_node)->set_size(get_size()->clone());
    }
    if(get_var()) {
        cast_to<PartselectMinusIndexed>(new_node)->set_var(get_var()->clone());
    }
}
