...
Input::Input(const std::string &filename, uint32_t line) : IODir(filename, line)
{
    set_node_type(NodeType::Input);
    set_node_categories({NodeType::IODir, NodeType::Node});
}

Input::Input(const Width::ListPtr widths, const std::string &name, const bool &sign,
...
bool Input::operator!=(const Input &rhs) const { return !(operator==(rhs)); }

bool Input::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Input::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Input>(new_node)->set_widths(Width::clone_list(get_widths()));
}
