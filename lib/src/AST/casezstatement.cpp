...
CasezStatement::CasezStatement(const Node::Ptr comp, const Case::ListPtr caselist,
...
bool CasezStatement::operator!=(const CasezStatement &rhs) const { return !(operator==(rhs)); }

bool CasezStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool CasezStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const Case::Ptr &lnode : *get_caselist()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "CasezStatement::replace matches multiple times "
                                       "(list(Case)::caselist)";
                    }
                    if(new_node) {
...
{
    if(get_comp()) {
        cast_to<CasezStatement>(new_node)->set_comp(get_comp()->clone());
    }
    cast_to<CasezStatement>(new_node)->set_caselist(Case::clone_list(get_caselist()));
}
