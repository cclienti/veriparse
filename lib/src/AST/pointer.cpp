...
Pointer::Pointer(const std::string &filename, uint32_t line) : Indirect(filename, line)
{
    set_node_type(NodeType::Pointer);
    set_node_categories({NodeType::Indirect, NodeType::Node});
}

Pointer::Pointer(const Node::Ptr ptr, const Node::Ptr var, const std::string &filename,
                 uint32_t line)
...
bool Pointer::operator!=(const Pointer &rhs) const { return !(operator==(rhs)); }

bool Pointer::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Pointer::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_ptr()) {
        cast_to<Pointer>(new_node)->set_ptr(get_ptr()->clone());
    }
    if(get_var()) {
        cast_to<Pointer>(new_node)->set_var(get_var()->clone());
    }
}
