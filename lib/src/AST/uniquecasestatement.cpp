...
UniqueCaseStatement::UniqueCaseStatement(const Node::Ptr comp, const Case::ListPtr caselist, const std::string &filename, uint32_t line):
...
bool UniqueCaseStatement::remove(Node::Ptr node) {
		...
		for (const Case::Ptr &lnode : *get_caselist()) {
			if (lnode) {
				if (lnode != node) {
					...
					if (found) {
						LOG_WARNING << *this << ", "
										<< "UniqueCaseStatement::replace matches multiple times (list(Case)::caselist)";
					}
					if(new_node) {
...
void UniqueCaseStatement::clone_children(Node::Ptr new_node) const {
	if (get_comp()) {
		cast_to<UniqueCaseStatement>(new_node)->set_comp
			(get_comp()->clone());
	}
	cast_to<UniqueCaseStatement>(new_node)->set_caselist
