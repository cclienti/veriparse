...
GreaterThan::GreaterThan(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::GreaterThan);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

GreaterThan::GreaterThan(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
                         uint32_t line)
...
bool GreaterThan::operator!=(const GreaterThan &rhs) const { return !(operator==(rhs)); }

bool GreaterThan::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool GreaterThan::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<GreaterThan>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<GreaterThan>(new_node)->set_right(get_right()->clone());
    }
}
