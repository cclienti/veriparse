...
LessEq::LessEq(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::LessEq);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

LessEq::LessEq(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
               uint32_t line)
...
bool LessEq::operator!=(const LessEq &rhs) const { return !(operator==(rhs)); }

bool LessEq::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool LessEq::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<LessEq>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<LessEq>(new_node)->set_right(get_right()->clone());
    }
}
