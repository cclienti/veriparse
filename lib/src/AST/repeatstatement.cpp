...
RepeatStatement::RepeatStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::RepeatStatement);
    set_node_categories({NodeType::Node});
}

RepeatStatement::RepeatStatement(const Node::Ptr times, const Node::Ptr statement,
...
bool RepeatStatement::operator!=(const RepeatStatement &rhs) const { return !(operator==(rhs)); }

bool RepeatStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool RepeatStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_times()) {
        cast_to<RepeatStatement>(new_node)->set_times(get_times()->clone());
    }
    if(get_statement()) {
        cast_to<RepeatStatement>(new_node)->set_statement(get_statement()->clone());
    }
}
