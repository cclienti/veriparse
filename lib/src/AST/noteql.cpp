...
NotEql::NotEql(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::NotEql);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

NotEql::NotEql(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
               uint32_t line)
...
bool NotEql::operator!=(const NotEql &rhs) const { return !(operator==(rhs)); }

bool NotEql::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool NotEql::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<NotEql>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<NotEql>(new_node)->set_right(get_right()->clone());
    }
}
