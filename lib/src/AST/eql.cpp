...
Eql::Eql(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Eql::operator!=(const Eql &rhs) const { return !(operator==(rhs)); }

bool Eql::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Eql::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Eql>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Eql>(new_node)->set_right(get_right()->clone());
    }
}
