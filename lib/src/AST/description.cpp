...
Description::Description(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Description);
    set_node_categories({NodeType::Node});
}

Description::Description(const Node::ListPtr definitions, const std::string &filename,
                         uint32_t line)
...
bool Description::operator!=(const Description &rhs) const { return !(operator==(rhs)); }

bool Description::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Description::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool Description::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_definitions()) {
        ...
        for(const Node::Ptr &lnode : *get_definitions()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "Description::replace matches multiple times "
...
{
    cast_to<Description>(new_node)->set_definitions(Node::clone_list(get_definitions()));
}
