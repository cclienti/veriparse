...
Identifier::Identifier(const IdentifierScope::Ptr scope, const std::string &name,
                       const std::string &package, const std::string &filename, uint32_t line)
    : Node(filename, line), m_scope(scope), m_name(name), m_package(package)
{
    set_node_type(NodeType::Identifier);
    set_node_categories({NodeType::Node});
}
...
{
    if(get_scope()) {
        cast_to<Identifier>(new_node)->set_scope(cast_to<IdentifierScope>(get_scope()->clone()));
    }
}
