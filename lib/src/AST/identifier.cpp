...
Identifier::Identifier(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Identifier);
    set_node_categories({NodeType::Node});
}

Identifier::Identifier(const ScopeName::ListPtr scope, const HierName::Ptr hier,
                       const std::string &name, const std::string &filename, uint32_t line)
    : Node(filename, line), m_scope(scope), m_hier(hier), m_name(name)
{
    set_node_type(NodeType::Identifier);
    set_node_categories({NodeType::Node});
}
...
bool Identifier::operator!=(const Identifier &rhs) const { return !(operator==(rhs)); }

bool Identifier::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Identifier::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool Identifier::replace(Node::Ptr node, Node::Ptr new_node)
{
        ...
        for(const ScopeName::Ptr &lnode : *get_scope()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "Identifier::replace matches multiple times "
                ...
                LOG_WARNING << *this << ", "
                            << "found an empty node during Identifier::replace "
    ...
    if(get_hier()) {
        if(get_hier() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "Identifier::replace matches multiple times (HierName::hier)";
            }
...
{
    cast_to<Identifier>(new_node)->set_scope(ScopeName::clone_list(get_scope()));
    if(get_hier()) {
        cast_to<Identifier>(new_node)->set_hier(cast_to<HierName>(get_hier()->clone()));
    }
}
