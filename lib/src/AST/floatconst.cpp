...
FloatConst::FloatConst(const std::string &filename, uint32_t line) : Constant(filename, line)
{
...
bool FloatConst::operator!=(const FloatConst &rhs) const { return !(operator==(rhs)); }

bool FloatConst::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool FloatConst::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
