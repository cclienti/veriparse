...
Initial::Initial(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Initial::operator!=(const Initial &rhs) const { return !(operator==(rhs)); }

bool Initial::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Initial::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_statement()) {
        cast_to<Initial>(new_node)->set_statement(get_statement()->clone());
    }
}
