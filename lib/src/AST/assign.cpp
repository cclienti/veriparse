...
Assign::Assign(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Assign);
    set_node_categories({NodeType::Node});
}

Assign::Assign(const Lvalue::Ptr left, const Rvalue::Ptr right, const DelayStatement::Ptr ldelay,
               const DelayStatement::Ptr rdelay, const std::string &filename, uint32_t line)
...
bool Assign::operator!=(const Assign &rhs) const { return !(operator==(rhs)); }

bool Assign::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Assign::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Assign>(new_node)->set_left(cast_to<Lvalue>(get_left()->clone()));
    }
    if(get_right()) {
        cast_to<Assign>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
    if(get_ldelay()) {
        cast_to<Assign>(new_node)->set_ldelay(cast_to<DelayStatement>(get_ldelay()->clone()));
    }
    if(get_rdelay()) {
        cast_to<Assign>(new_node)->set_rdelay(cast_to<DelayStatement>(get_rdelay()->clone()));
    }
}
