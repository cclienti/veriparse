...
Times::Times(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::Times);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

Times::Times(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
             uint32_t line)
...
bool Times::operator!=(const Times &rhs) const { return !(operator==(rhs)); }

bool Times::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Times::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Times>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Times>(new_node)->set_right(get_right()->clone());
    }
}
