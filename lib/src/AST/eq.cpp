...
Eq::Eq(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Eq::operator!=(const Eq &rhs) const { return !(operator==(rhs)); }

bool Eq::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Eq::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Eq>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Eq>(new_node)->set_right(get_right()->clone());
    }
}
