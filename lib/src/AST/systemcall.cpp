...
SystemCall::SystemCall(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::SystemCall);
    set_node_categories({NodeType::Node});
}

SystemCall::SystemCall(const Node::ListPtr args, const std::string &syscall,
...
bool SystemCall::operator!=(const SystemCall &rhs) const { return !(operator==(rhs)); }

bool SystemCall::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool SystemCall::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<SystemCall>(new_node)->set_args(Node::clone_list(get_args()));
}
