...
LessThan::LessThan(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::LessThan);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

LessThan::LessThan(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
                   uint32_t line)
...
bool LessThan::operator!=(const LessThan &rhs) const { return !(operator==(rhs)); }

bool LessThan::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool LessThan::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<LessThan>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<LessThan>(new_node)->set_right(get_right()->clone());
    }
}
