...
Instance::Instance(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Instance::operator!=(const Instance &rhs) const { return !(operator==(rhs)); }

bool Instance::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Instance::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const ParamArg::Ptr &lnode : *get_parameterlist()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "Instance::replace matches multiple times "
