...
WhileStatement::WhileStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::WhileStatement);
    set_node_categories({NodeType::Node});
}

WhileStatement::WhileStatement(const Node::Ptr cond, const Node::Ptr statement,
...
bool WhileStatement::operator!=(const WhileStatement &rhs) const { return !(operator==(rhs)); }

bool WhileStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool WhileStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_cond()) {
        cast_to<WhileStatement>(new_node)->set_cond(get_cond()->clone());
    }
    if(get_statement()) {
        cast_to<WhileStatement>(new_node)->set_statement(get_statement()->clone());
    }
}
