...
Uplus::Uplus(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Uplus::operator!=(const Uplus &rhs) const { return !(operator==(rhs)); }

bool Uplus::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Uplus::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Uplus>(new_node)->set_right(get_right()->clone());
    }
}
