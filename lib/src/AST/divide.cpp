...
Divide::Divide(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::Divide);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

Divide::Divide(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
               uint32_t line)
...
bool Divide::operator!=(const Divide &rhs) const { return !(operator==(rhs)); }

bool Divide::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Divide::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Divide>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Divide>(new_node)->set_right(get_right()->clone());
    }
}
