...
TypeOpType::TypeOpType(const DataType::Ptr type, const Dimension::ListPtr packed_dims,
                       const DataType::SigningEnum &signing, const std::string &filename,
                       uint32_t line)
    : DataType(packed_dims, signing, filename, line), m_type(type)
{
    set_node_type(NodeType::TypeOpType);
    set_node_categories({NodeType::DataType, NodeType::Node});
}
...
bool TypeOpType::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_type()) {
        if(get_type() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "TypeOpType::replace matches multiple times (DataType::type)";
            }
...
{
    if(get_type()) {
        cast_to<TypeOpType>(new_node)->set_type(cast_to<DataType>(get_type()->clone()));
    }
