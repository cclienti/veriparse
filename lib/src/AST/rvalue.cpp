...
Rvalue::Rvalue(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Rvalue::operator!=(const Rvalue &rhs) const { return !(operator==(rhs)); }

bool Rvalue::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Rvalue::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_var()) {
        cast_to<Rvalue>(new_node)->set_var(get_var()->clone());
    }
}
