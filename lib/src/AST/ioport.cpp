...
Ioport::Ioport(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Ioport);
    set_node_categories({NodeType::Node});
}

Ioport::Ioport(const IODir::Ptr first, const Variable::Ptr second, const std::string &filename,
               uint32_t line)
...
bool Ioport::operator!=(const Ioport &rhs) const { return !(operator==(rhs)); }

bool Ioport::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Ioport::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_first()) {
        cast_to<Ioport>(new_node)->set_first(cast_to<IODir>(get_first()->clone()));
    }
    if(get_second()) {
        cast_to<Ioport>(new_node)->set_second(cast_to<Variable>(get_second()->clone()));
    }
}
