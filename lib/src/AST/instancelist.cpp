...
Instancelist::Instancelist(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Instancelist);
    set_node_categories({NodeType::Node});
}

Instancelist::Instancelist(const ParamArg::ListPtr parameterlist, const Instance::ListPtr instances,
                           const std::string &module, const std::string &filename, uint32_t line)
...
bool Instancelist::operator!=(const Instancelist &rhs) const { return !(operator==(rhs)); }

bool Instancelist::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Instancelist::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const ParamArg::Ptr &lnode : *get_parameterlist()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "Instancelist::replace matches multiple times "
...
{
    cast_to<Instancelist>(new_node)->set_parameterlist(ParamArg::clone_list(get_parameterlist()));
    cast_to<Instancelist>(new_node)->set_instances(Instance::clone_list(get_instances()));
}
