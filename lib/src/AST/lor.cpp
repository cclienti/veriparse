...
Lor::Lor(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Lor::operator!=(const Lor &rhs) const { return !(operator==(rhs)); }

bool Lor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Lor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Lor>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Lor>(new_node)->set_right(get_right()->clone());
    }
}
