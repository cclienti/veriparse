...
Srl::Srl(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Srl::operator!=(const Srl &rhs) const { return !(operator==(rhs)); }

bool Srl::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Srl::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Srl>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Srl>(new_node)->set_right(get_right()->clone());
    }
}
