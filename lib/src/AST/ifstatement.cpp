...
IfStatement::IfStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::IfStatement);
    set_node_categories({NodeType::Node});
}

IfStatement::IfStatement(const Node::Ptr cond, const Node::Ptr true_statement,
                         const Node::Ptr false_statement, const std::string &filename,
                         uint32_t line)
    : Node(filename, line), m_cond(cond), m_true_statement(true_statement),
...
bool IfStatement::operator!=(const IfStatement &rhs) const { return !(operator==(rhs)); }

bool IfStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool IfStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_cond()) {
        cast_to<IfStatement>(new_node)->set_cond(get_cond()->clone());
    }
    if(get_true_statement()) {
        cast_to<IfStatement>(new_node)->set_true_statement(get_true_statement()->clone());
    }
    if(get_false_statement()) {
        cast_to<IfStatement>(new_node)->set_false_statement(get_false_statement()->clone());
    }
}
