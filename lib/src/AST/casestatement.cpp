...
CaseStatement::CaseStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::CaseStatement);
    set_node_categories({NodeType::Node});
}

CaseStatement::CaseStatement(const Node::Ptr comp, const Case::ListPtr caselist,
...
bool CaseStatement::operator!=(const CaseStatement &rhs) const { return !(operator==(rhs)); }

bool CaseStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool CaseStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const Case::Ptr &lnode : *get_caselist()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "CaseStatement::replace matches multiple times "
                                       "(list(Case)::caselist)";
                    }
                    if(new_node) {
...
{
    if(get_comp()) {
        cast_to<CaseStatement>(new_node)->set_comp(get_comp()->clone());
    }
    cast_to<CaseStatement>(new_node)->set_caselist(Case::clone_list(get_caselist()));
}
