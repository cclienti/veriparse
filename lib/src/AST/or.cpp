...
Or::Or(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Or::operator!=(const Or &rhs) const { return !(operator==(rhs)); }

bool Or::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Or::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Or>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Or>(new_node)->set_right(get_right()->clone());
    }
}
