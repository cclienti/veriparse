...
Integer::Integer(const std::string &filename, uint32_t line) : Variable(filename, line)
{
    set_node_type(NodeType::Integer);
    set_node_categories({NodeType::Variable, NodeType::VariableBase, NodeType::Node});
}

Integer::Integer(const Length::ListPtr lengths, const Rvalue::Ptr right, const std::string &name,
...
bool Integer::operator!=(const Integer &rhs) const { return !(operator==(rhs)); }

bool Integer::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Integer::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Integer>(new_node)->set_lengths(Length::clone_list(get_lengths()));
    if(get_right()) {
        cast_to<Integer>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
}
