...
Reg::Reg(const std::string &filename, uint32_t line) : Variable(filename, line)
{
    set_node_type(NodeType::Reg);
    set_node_categories({NodeType::Variable, NodeType::VariableBase, NodeType::Node});
}

Reg::Reg(const Width::ListPtr widths, const Length::ListPtr lengths, const Rvalue::Ptr right,
...
bool Reg::operator!=(const Reg &rhs) const { return !(operator==(rhs)); }

bool Reg::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Reg::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Reg>(new_node)->set_widths(Width::clone_list(get_widths()));
    cast_to<Reg>(new_node)->set_lengths(Length::clone_list(get_lengths()));
    if(get_right()) {
        cast_to<Reg>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
}
