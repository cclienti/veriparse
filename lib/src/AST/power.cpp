...
Power::Power(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::Power);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

Power::Power(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
             uint32_t line)
...
bool Power::operator!=(const Power &rhs) const { return !(operator==(rhs)); }

bool Power::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Power::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Power>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Power>(new_node)->set_right(get_right()->clone());
    }
}
