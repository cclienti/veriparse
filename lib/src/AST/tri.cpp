...
Tri::Tri(const std::string &filename, uint32_t line) : Net(filename, line)
{
...
Tri::Tri(const Width::ListPtr widths, const DelayStatement::Ptr ldelay,
...
bool Tri::operator!=(const Tri &rhs) const { return !(operator==(rhs)); }

bool Tri::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Tri::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool Tri::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_type()) {
        if(get_type() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "Tri::replace matches multiple times (Node::type)";
            }
            set_type(new_node);
            found = true;
        }
...
{
    cast_to<Tri>(new_node)->set_widths(Width::clone_list(get_widths()));
    if(get_ldelay()) {
        cast_to<Tri>(new_node)->set_ldelay(cast_to<DelayStatement>(get_ldelay()->clone()));
    }
    if(get_rdelay()) {
        cast_to<Tri>(new_node)->set_rdelay(cast_to<DelayStatement>(get_rdelay()->clone()));
    }
    if(get_type()) {
        cast_to<Tri>(new_node)->set_type(get_type()->clone());
    }
    cast_to<Tri>(new_node)->set_lengths(Length::clone_list(get_lengths()));
    if(get_right()) {
        cast_to<Tri>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
}
