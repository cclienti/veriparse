...
SingleStatement::SingleStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::SingleStatement);
    set_node_categories({NodeType::Node});
}

SingleStatement::SingleStatement(const Node::Ptr statement, const DelayStatement::Ptr delay,
                                 const std::string &scope, const std::string &filename,
                                 uint32_t line)
...
bool SingleStatement::operator!=(const SingleStatement &rhs) const { return !(operator==(rhs)); }

bool SingleStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool SingleStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_statement()) {
        cast_to<SingleStatement>(new_node)->set_statement(get_statement()->clone());
    }
    if(get_delay()) {
