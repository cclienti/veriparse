...
Variable::Variable(const std::string &filename, uint32_t line) : VariableBase(filename, line)
{
    set_node_type(NodeType::Variable);
    set_node_categories({NodeType::VariableBase, NodeType::Node});
}

Variable::Variable(const Length::ListPtr lengths, const Rvalue::Ptr right, const std::string &name,
...
bool Variable::operator!=(const Variable &rhs) const { return !(operator==(rhs)); }

bool Variable::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Variable::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Variable>(new_node)->set_lengths(Length::clone_list(get_lengths()));
    if(get_right()) {
        cast_to<Variable>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
}
