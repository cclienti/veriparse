...
AlwaysComb::AlwaysComb(const std::string &filename, uint32_t line):
...
AlwaysComb::AlwaysComb(const Senslist::Ptr senslist, const Node::Ptr statement, const std::string &filename, uint32_t line):
...
bool AlwaysComb::remove(Node::Ptr node) {
...
void AlwaysComb::clone_children(Node::Ptr new_node) const {
	if (get_senslist()) {
		cast_to<AlwaysComb>(new_node)->set_senslist
			(cast_to<Senslist>(get_senslist()->clone()));
	}
	if (get_statement()) {
		cast_to<AlwaysComb>(new_node)->set_statement
			(get_statement()->clone());
	}
}
