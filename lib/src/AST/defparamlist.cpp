...
Defparamlist::Defparamlist(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Defparamlist::operator!=(const Defparamlist &rhs) const { return !(operator==(rhs)); }

bool Defparamlist::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Defparamlist::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const Defparam::Ptr &lnode : *get_list()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "Defparamlist::replace matches multiple times "
                                       "(list(Defparam)::list)";
                    }
                    if(new_node) {
...
{
    cast_to<Defparamlist>(new_node)->set_list(Defparam::clone_list(get_list()));
}
