...
Var::Var(const Rvalue::Ptr init, const DataType::Ptr type, const Dimension::ListPtr unpacked_dims,
    ...
    : Declaration(type, unpacked_dims, name, filename, line), m_init(init), m_is_var(is_var),
      m_is_const(is_const), m_lifetime(lifetime)
{
    set_node_type(NodeType::Var);
    set_node_categories({NodeType::Declaration, NodeType::Node});
}
