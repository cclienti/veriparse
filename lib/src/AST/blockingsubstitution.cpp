...
BlockingSubstitution::BlockingSubstitution(const Lvalue::Ptr left, const Rvalue::Ptr right,
                                           const DelayStatement::Ptr ldelay,
                                           const DelayStatement::Ptr rdelay,
...
bool BlockingSubstitution::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool BlockingSubstitution::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool BlockingSubstitution::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_ldelay()) {
        if(get_ldelay() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "BlockingSubstitution::replace matches multiple times "
                               "(DelayStatement::ldelay)";
            }
...
{
    if(get_left()) {
        cast_to<BlockingSubstitution>(new_node)->set_left(cast_to<Lvalue>(get_left()->clone()));
    }
    if(get_right()) {
        cast_to<BlockingSubstitution>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
    if(get_ldelay()) {
