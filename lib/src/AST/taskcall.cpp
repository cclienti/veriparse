...
TaskCall::TaskCall(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::TaskCall);
    set_node_categories({NodeType::Node});
}

TaskCall::TaskCall(const Node::ListPtr args, const std::string &name, const std::string &package,
