...
Operator::Operator(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Operator);
    set_node_categories({NodeType::Node});
}

Operator::Operator(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
                   uint32_t line)
...
bool Operator::operator!=(const Operator &rhs) const { return !(operator==(rhs)); }

bool Operator::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Operator::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Operator>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Operator>(new_node)->set_right(get_right()->clone());
    }
}
