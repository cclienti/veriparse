...
Pragmalist::Pragmalist(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Pragmalist);
    set_node_categories({NodeType::Node});
}

Pragmalist::Pragmalist(const Pragma::ListPtr pragmas, const Node::ListPtr statements,
...
bool Pragmalist::operator!=(const Pragmalist &rhs) const { return !(operator==(rhs)); }

bool Pragmalist::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Pragmalist::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
        ...
        for(const Node::Ptr &lnode : *get_statements()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "Pragmalist::replace matches multiple times "
...
{
    cast_to<Pragmalist>(new_node)->set_pragmas(Pragma::clone_list(get_pragmas()));
    cast_to<Pragmalist>(new_node)->set_statements(Node::clone_list(get_statements()));
}
