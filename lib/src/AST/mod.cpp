...
Mod::Mod(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Mod::operator!=(const Mod &rhs) const { return !(operator==(rhs)); }

bool Mod::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Mod::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Mod>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Mod>(new_node)->set_right(get_right()->clone());
    }
}
