...
Ulnot::Ulnot(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Ulnot::operator!=(const Ulnot &rhs) const { return !(operator==(rhs)); }

bool Ulnot::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Ulnot::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Ulnot>(new_node)->set_right(get_right()->clone());
    }
}
