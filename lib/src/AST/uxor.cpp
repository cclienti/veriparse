...
Uxor::Uxor(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Uxor::operator!=(const Uxor &rhs) const { return !(operator==(rhs)); }

bool Uxor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Uxor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Uxor>(new_node)->set_right(get_right()->clone());
    }
}
