...
IntConst::IntConst(const std::string &filename, uint32_t line) : Constant(filename, line)
{
...
bool IntConst::operator!=(const IntConst &rhs) const { return !(operator==(rhs)); }

bool IntConst::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool IntConst::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
