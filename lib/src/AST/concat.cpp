...
Concat::Concat(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Concat::operator!=(const Concat &rhs) const { return !(operator==(rhs)); }

bool Concat::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Concat::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Concat>(new_node)->set_list(Node::clone_list(get_list()));
}
