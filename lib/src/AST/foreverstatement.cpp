...
ForeverStatement::ForeverStatement(const Node::Ptr statement, const std::string &filename,
                                   uint32_t line)
...
bool ForeverStatement::operator!=(const ForeverStatement &rhs) const { return !(operator==(rhs)); }

bool ForeverStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool ForeverStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_statement()) {
        cast_to<ForeverStatement>(new_node)->set_statement(get_statement()->clone());
    }
}
