...
AlwaysLatch::AlwaysLatch(const std::string &filename, uint32_t line) : Always(filename, line)
{
    set_node_type(NodeType::AlwaysLatch);
    set_node_categories({NodeType::Always, NodeType::Node});
}

AlwaysLatch::AlwaysLatch(const Senslist::Ptr senslist, const Node::Ptr statement,
...
bool AlwaysLatch::operator!=(const AlwaysLatch &rhs) const { return !(operator==(rhs)); }

bool AlwaysLatch::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool AlwaysLatch::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_senslist()) {
        cast_to<AlwaysLatch>(new_node)->set_senslist(cast_to<Senslist>(get_senslist()->clone()));
    }
    if(get_statement()) {
        cast_to<AlwaysLatch>(new_node)->set_statement(get_statement()->clone());
    }
}
