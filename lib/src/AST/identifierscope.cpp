...
IdentifierScope::IdentifierScope(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::IdentifierScope);
    set_node_categories({NodeType::Node});
}

IdentifierScope::IdentifierScope(const IdentifierScopeLabel::ListPtr labellist,
...
bool IdentifierScope::operator!=(const IdentifierScope &rhs) const { return !(operator==(rhs)); }

bool IdentifierScope::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool IdentifierScope::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool IdentifierScope::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_labellist()) {
        ...
        for(const IdentifierScopeLabel::Ptr &lnode : *get_labellist()) {
            if(lnode) {
                if(lnode != node) {
                    ...
                    if(found) {
                        LOG_WARNING << *this << ", "
                                    << "IdentifierScope::replace matches multiple times "
                                       "(list(IdentifierScopeLabel)::labellist)";
                    }
                    if(new_node) {
