...
Node::Node(const std::string &filename, uint32_t line)
    : m_node_type(NodeType::Node), m_filename(filename), m_line(line)
{
}
...
bool Node::remove(Node::Ptr node) { return false; }

bool Node::replace(Node::Ptr node, Node::Ptr new_node) { return false; }

bool Node::replace(Node::Ptr node, Node::ListPtr new_nodes) { return false; }
...
bool Node::is_equal(const Node &node, bool full_check) const
{
    ...
    for(; (it1 != node_children1->end()) && (it2 != node_children2->end()); ++it1, ++it2) {
        ...
        if(!(ref2.is_equal(ref1, full_check))) {
            // Child are different
            LOG_DEBUG << "Node::is_equal: check children: diff between \"" << ref2.get_filename()
                      << ":" << ref2.get_line() << "\""
    ...
    if((it1 == node_children1->end()) && (it2 == node_children2->end())) {
        ...
        for(; it1 != node_children1->end(); ++it1) {
            LOG_DEBUG << "Node::is_equal: check children: remaining not checked child in the list "
                         "in the node: "
                      << *it1;
        }
...
Node::ListPtr Node::get_children(void) const { return std::make_shared<Node::List>(); }
