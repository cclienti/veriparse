...
VariableBase::VariableBase(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool VariableBase::operator!=(const VariableBase &rhs) const { return !(operator==(rhs)); }

bool VariableBase::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool VariableBase::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
