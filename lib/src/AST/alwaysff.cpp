...
AlwaysFF::AlwaysFF(const std::string &filename, uint32_t line) : Always(filename, line)
{
    set_node_type(NodeType::AlwaysFF);
    set_node_categories({NodeType::Always, NodeType::Node});
}

AlwaysFF::AlwaysFF(const Senslist::Ptr senslist, const Node::Ptr statement,
...
bool AlwaysFF::operator!=(const AlwaysFF &rhs) const { return !(operator==(rhs)); }

bool AlwaysFF::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool AlwaysFF::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_senslist()) {
        cast_to<AlwaysFF>(new_node)->set_senslist(cast_to<Senslist>(get_senslist()->clone()));
    }
    if(get_statement()) {
        cast_to<AlwaysFF>(new_node)->set_statement(get_statement()->clone());
    }
}
