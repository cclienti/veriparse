...
Partselect::Partselect(const std::string &filename, uint32_t line) : Indirect(filename, line)
{
    set_node_type(NodeType::Partselect);
    set_node_categories({NodeType::Indirect, NodeType::Node});
}

Partselect::Partselect(const Node::Ptr msb, const Node::Ptr lsb, const Node::Ptr var,
...
bool Partselect::operator!=(const Partselect &rhs) const { return !(operator==(rhs)); }

bool Partselect::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Partselect::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_msb()) {
        cast_to<Partselect>(new_node)->set_msb(get_msb()->clone());
    }
    if(get_lsb()) {
        cast_to<Partselect>(new_node)->set_lsb(get_lsb()->clone());
    }
    if(get_var()) {
        cast_to<Partselect>(new_node)->set_var(get_var()->clone());
    }
}
