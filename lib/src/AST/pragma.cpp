...
Pragma::Pragma(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Pragma);
    set_node_categories({NodeType::Node});
}

Pragma::Pragma(const Node::Ptr expression, const std::string &name, const std::string &filename,
               uint32_t line)
...
bool Pragma::operator!=(const Pragma &rhs) const { return !(operator==(rhs)); }

bool Pragma::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Pragma::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_expression()) {
        cast_to<Pragma>(new_node)->set_expression(get_expression()->clone());
    }
}
