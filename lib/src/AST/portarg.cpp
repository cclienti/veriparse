...
PortArg::PortArg(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::PortArg);
    set_node_categories({NodeType::Node});
}

PortArg::PortArg(const Node::Ptr value, const std::string &name, const std::string &filename,
                 uint32_t line)
...
bool PortArg::operator!=(const PortArg &rhs) const { return !(operator==(rhs)); }

bool PortArg::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool PortArg::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_value()) {
        cast_to<PortArg>(new_node)->set_value(get_value()->clone());
    }
}
