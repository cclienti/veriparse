...
Unand::Unand(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Unand::operator!=(const Unand &rhs) const { return !(operator==(rhs)); }

bool Unand::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Unand::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Unand>(new_node)->set_right(get_right()->clone());
    }
}
