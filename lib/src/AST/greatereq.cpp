...
GreaterEq::GreaterEq(const std::string &filename, uint32_t line) : Operator(filename, line)
{
    set_node_type(NodeType::GreaterEq);
    set_node_categories({NodeType::Operator, NodeType::Node});
}

GreaterEq::GreaterEq(const Node::Ptr left, const Node::Ptr right, const std::string &filename,
                     uint32_t line)
...
bool GreaterEq::operator!=(const GreaterEq &rhs) const { return !(operator==(rhs)); }

bool GreaterEq::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool GreaterEq::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<GreaterEq>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<GreaterEq>(new_node)->set_right(get_right()->clone());
    }
}
