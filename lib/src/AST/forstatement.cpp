...
ForStatement::ForStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::ForStatement);
    set_node_categories({NodeType::Node});
}

ForStatement::ForStatement(const BlockingSubstitution::Ptr pre, const Node::Ptr cond,
                           const BlockingSubstitution::Ptr post, const Node::Ptr statement,
...
bool ForStatement::operator!=(const ForStatement &rhs) const { return !(operator==(rhs)); }

bool ForStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool ForStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_pre()) {
        cast_to<ForStatement>(new_node)->set_pre(cast_to<BlockingSubstitution>(get_pre()->clone()));
    }
    if(get_cond()) {
        cast_to<ForStatement>(new_node)->set_cond(get_cond()->clone());
    }
    if(get_post()) {
    ...
    if(get_statement()) {
        cast_to<ForStatement>(new_node)->set_statement(get_statement()->clone());
    }
}
