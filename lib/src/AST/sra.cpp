...
Sra::Sra(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Sra::operator!=(const Sra &rhs) const { return !(operator==(rhs)); }

bool Sra::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Sra::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Sra>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Sra>(new_node)->set_right(get_right()->clone());
    }
}
