...
Unot::Unot(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Unot::operator!=(const Unot &rhs) const { return !(operator==(rhs)); }

bool Unot::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Unot::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Unot>(new_node)->set_right(get_right()->clone());
    }
}
