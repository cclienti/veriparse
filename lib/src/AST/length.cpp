...
Length::Length(const std::string &filename, uint32_t line) : Width(filename, line)
{
...
bool Length::operator!=(const Length &rhs) const { return !(operator==(rhs)); }

bool Length::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Length::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_msb()) {
        cast_to<Length>(new_node)->set_msb(get_msb()->clone());
    }
    if(get_lsb()) {
        cast_to<Length>(new_node)->set_lsb(get_lsb()->clone());
    }
}
