...
IODir::IODir(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::IODir);
    set_node_categories({NodeType::Node});
}

IODir::IODir(const Width::ListPtr widths, const std::string &name, const bool &sign,
...
bool IODir::operator!=(const IODir &rhs) const { return !(operator==(rhs)); }

bool IODir::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool IODir::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<IODir>(new_node)->set_widths(Width::clone_list(get_widths()));
}
