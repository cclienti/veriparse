...
Inout::Inout(const std::string &filename, uint32_t line) : IODir(filename, line)
{
    set_node_type(NodeType::Inout);
    set_node_categories({NodeType::IODir, NodeType::Node});
}

Inout::Inout(const Width::ListPtr widths, const std::string &name, const bool &sign,
...
bool Inout::operator!=(const Inout &rhs) const { return !(operator==(rhs)); }

bool Inout::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Inout::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Inout>(new_node)->set_widths(Width::clone_list(get_widths()));
}
