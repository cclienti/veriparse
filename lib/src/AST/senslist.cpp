...
Senslist::Senslist(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Senslist::operator!=(const Senslist &rhs) const { return !(operator==(rhs)); }

bool Senslist::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Senslist::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Senslist>(new_node)->set_list(Sens::clone_list(get_list()));
}
