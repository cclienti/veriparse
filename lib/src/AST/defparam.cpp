...
Defparam::Defparam(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Defparam);
    set_node_categories({NodeType::Node});
}

Defparam::Defparam(const Identifier::Ptr identifier, const Rvalue::Ptr right,
...
bool Defparam::operator!=(const Defparam &rhs) const { return !(operator==(rhs)); }

bool Defparam::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Defparam::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_identifier()) {
        cast_to<Defparam>(new_node)->set_identifier(cast_to<Identifier>(get_identifier()->clone()));
    }
    if(get_right()) {
        cast_to<Defparam>(new_node)->set_right(cast_to<Rvalue>(get_right()->clone()));
    }
}
