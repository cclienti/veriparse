...
IdentifierScopeLabel::IdentifierScopeLabel(const Node::Ptr loop, const std::string &scope,
...
bool IdentifierScopeLabel::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool IdentifierScopeLabel::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_loop()) {
        cast_to<IdentifierScopeLabel>(new_node)->set_loop(get_loop()->clone());
    }
}
