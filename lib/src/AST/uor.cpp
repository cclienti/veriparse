...
Uor::Uor(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Uor::operator!=(const Uor &rhs) const { return !(operator==(rhs)); }

bool Uor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Uor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Uor>(new_node)->set_right(get_right()->clone());
    }
}
