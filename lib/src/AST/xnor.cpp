...
Xnor::Xnor(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Xnor::operator!=(const Xnor &rhs) const { return !(operator==(rhs)); }

bool Xnor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Xnor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Xnor>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Xnor>(new_node)->set_right(get_right()->clone());
    }
}
