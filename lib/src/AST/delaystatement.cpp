...
DelayStatement::DelayStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool DelayStatement::operator!=(const DelayStatement &rhs) const { return !(operator==(rhs)); }

bool DelayStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool DelayStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_delay()) {
        cast_to<DelayStatement>(new_node)->set_delay(get_delay()->clone());
    }
}
