...
Xor::Xor(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Xor::operator!=(const Xor &rhs) const { return !(operator==(rhs)); }

bool Xor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Xor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Xor>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Xor>(new_node)->set_right(get_right()->clone());
    }
}
