...
And::And(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool And::operator!=(const And &rhs) const { return !(operator==(rhs)); }

bool And::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool And::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<And>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<And>(new_node)->set_right(get_right()->clone());
    }
}
