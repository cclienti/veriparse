...
Repeat::Repeat(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Repeat);
    set_node_categories({NodeType::Node});
}

Repeat::Repeat(const Node::Ptr value, const Node::Ptr times, const std::string &filename,
               uint32_t line)
...
bool Repeat::operator!=(const Repeat &rhs) const { return !(operator==(rhs)); }

bool Repeat::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Repeat::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_value()) {
        cast_to<Repeat>(new_node)->set_value(get_value()->clone());
    }
    if(get_times()) {
        cast_to<Repeat>(new_node)->set_times(get_times()->clone());
    }
}
