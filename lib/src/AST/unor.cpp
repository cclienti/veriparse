...
Unor::Unor(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Unor::operator!=(const Unor &rhs) const { return !(operator==(rhs)); }

bool Unor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Unor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Unor>(new_node)->set_right(get_right()->clone());
    }
}
