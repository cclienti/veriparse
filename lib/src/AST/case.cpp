...
Case::Case(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Case);
    set_node_categories({NodeType::Node});
}

Case::Case(const Node::ListPtr cond, const Node::Ptr statement, const std::string &filename,
           uint32_t line)
...
bool Case::operator!=(const Case &rhs) const { return !(operator==(rhs)); }

bool Case::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Case::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Case>(new_node)->set_cond(Node::clone_list(get_cond()));
    if(get_statement()) {
        cast_to<Case>(new_node)->set_statement(get_statement()->clone());
    }
}
