...
IntConstN::IntConstN(const std::string &filename, uint32_t line) : Constant(filename, line)
{
    set_node_type(NodeType::IntConstN);
    set_node_categories({NodeType::Constant, NodeType::Node});
}

IntConstN::IntConstN(const int &base, const int &size, const bool &sign, const mpz_class &value,
...
bool IntConstN::operator!=(const IntConstN &rhs) const { return !(operator==(rhs)); }

bool IntConstN::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool IntConstN::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
