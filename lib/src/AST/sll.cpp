...
Sll::Sll(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Sll::operator!=(const Sll &rhs) const { return !(operator==(rhs)); }

bool Sll::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Sll::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Sll>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Sll>(new_node)->set_right(get_right()->clone());
    }
}
