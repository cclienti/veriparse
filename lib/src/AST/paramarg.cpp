...
ParamArg::ParamArg(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::ParamArg);
    set_node_categories({NodeType::Node});
}

ParamArg::ParamArg(const Node::Ptr value, const std::string &name, const std::string &filename,
                   uint32_t line)
...
bool ParamArg::operator!=(const ParamArg &rhs) const { return !(operator==(rhs)); }

bool ParamArg::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool ParamArg::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_value()) {
        cast_to<ParamArg>(new_node)->set_value(get_value()->clone());
    }
}
