...
UnaryOperator::UnaryOperator(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool UnaryOperator::operator!=(const UnaryOperator &rhs) const { return !(operator==(rhs)); }

bool UnaryOperator::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool UnaryOperator::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<UnaryOperator>(new_node)->set_right(get_right()->clone());
    }
}
