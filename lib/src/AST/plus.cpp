...
Plus::Plus(const std::string &filename, uint32_t line) : Operator(filename, line)
{
...
bool Plus::operator!=(const Plus &rhs) const { return !(operator==(rhs)); }

bool Plus::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Plus::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_left()) {
        cast_to<Plus>(new_node)->set_left(get_left()->clone());
    }
    if(get_right()) {
        cast_to<Plus>(new_node)->set_right(get_right()->clone());
    }
}
