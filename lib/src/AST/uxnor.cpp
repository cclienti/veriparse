...
Uxnor::Uxnor(const std::string &filename, uint32_t line) : UnaryOperator(filename, line)
{
...
bool Uxnor::operator!=(const Uxnor &rhs) const { return !(operator==(rhs)); }

bool Uxnor::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Uxnor::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_right()) {
        cast_to<Uxnor>(new_node)->set_right(get_right()->clone());
    }
}
