...
Supply0Net::Supply0Net(const std::string &filename, uint32_t line) : Net(filename, line)
{
...
Supply0Net &Supply0Net::operator=(const Supply0Net &rhs)
{
...
Node &Supply0Net::operator=(const Node &rhs)
{
    const Supply0Net &rhs_cast = static_cast<const Supply0Net &>(rhs);
    return static_cast<Node &>(operator=(rhs_cast));
}

bool Supply0Net::operator==(const Supply0Net &rhs) const
{
...
bool Supply0Net::operator==(const Node &rhs) const
{
    const Supply0Net &rhs_cast = static_cast<const Supply0Net &>(rhs);
    return operator==(rhs_cast);
}

bool Supply0Net::operator!=(const Supply0Net &rhs) const { return !(operator==(rhs)); }

bool Supply0Net::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Supply0Net::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
bool Supply0Net::replace(Node::Ptr node, Node::Ptr new_node)
{
    ...
    if(get_cont_assign()) {
        if(get_cont_assign() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "Supply0Net::replace matches multiple times (Rvalue::cont_assign)";
            }
    ...
    if(get_strength()) {
        if(get_strength() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "Supply0Net::replace matches multiple times (Strength::strength)";
            }
    ...
    if(get_ldelay()) {
        if(get_ldelay() == node) {
            if(found) {
                LOG_WARNING
                    << *this << ", "
                    << "Supply0Net::replace matches multiple times (DelayStatement::ldelay)";
            }
    ...
    if(get_rdelay()) {
        if(get_rdelay() == node) {
            if(found) {
                LOG_WARNING
                    << *this << ", "
                    << "Supply0Net::replace matches multiple times (DelayStatement::rdelay)";
            }
    ...
    if(get_type()) {
        if(get_type() == node) {
            if(found) {
                LOG_WARNING << *this << ", "
                            << "Supply0Net::replace matches multiple times (DataType::type)";
            }
...
bool Supply0Net::replace(Node::Ptr node, Node::ListPtr new_nodes)
{
    bool found = false;
    if(get_unpacked_dims()) {
        Dimension::ListPtr new_list = std::make_shared<Dimension::List>();
...
Supply0Net::ListPtr Supply0Net::clone_list(const ListPtr nodes)
{
    ListPtr list;
    if(nodes) {
        list = std::make_shared<List>();
        for(const Ptr &p : *nodes) {
            list->push_back(cast_to<Supply0Net>(p->clone()));
        }
    }
    return list;
}
...
void Supply0Net::clone_children(Node::Ptr new_node) const
{
    ...
    if(get_strength()) {
        cast_to<Supply0Net>(new_node)->set_strength(cast_to<Strength>(get_strength()->clone()));
    }
    if(get_ldelay()) {
        cast_to<Supply0Net>(new_node)->set_ldelay(cast_to<DelayStatement>(get_ldelay()->clone()));
    }
    if(get_rdelay()) {
        cast_to<Supply0Net>(new_node)->set_rdelay(cast_to<DelayStatement>(get_rdelay()->clone()));
    }
    if(get_type()) {
...
{
    Ptr p(new Supply0Net);
    return p;
}

std::ostream &operator<<(std::ostream &os, const Supply0Net &p)
{
    os << "Supply0Net: {";
...
std::ostream &operator<<(std::ostream &os, const Supply0Net::Ptr p)
{
    if(p) {
        os << *p;
    } else {
        os << "Supply0Net: {nullptr}";
    }
