...
Always::Always(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Always);
    set_node_categories({NodeType::Node});
}

Always::Always(const Senslist::Ptr senslist, const Node::Ptr statement, const std::string &filename,
               uint32_t line)
...
bool Always::operator!=(const Always &rhs) const { return !(operator==(rhs)); }

bool Always::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Always::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_senslist()) {
        cast_to<Always>(new_node)->set_senslist(cast_to<Senslist>(get_senslist()->clone()));
    }
    if(get_statement()) {
        cast_to<Always>(new_node)->set_statement(get_statement()->clone());
    }
}
