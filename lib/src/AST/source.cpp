...
Source::Source(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Source::operator!=(const Source &rhs) const { return !(operator==(rhs)); }

bool Source::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Source::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
