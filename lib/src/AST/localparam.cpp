...
Localparam::Localparam(const std::string &filename, uint32_t line) : Parameter(filename, line)
{
    set_node_type(NodeType::Localparam);
    set_node_categories({NodeType::Parameter, NodeType::Node});
}

Localparam::Localparam(const Node::Ptr value, const Width::ListPtr widths, const std::string &name,
                       const bool &sign, const Parameter::TypeEnum &type,
...
bool Localparam::operator!=(const Localparam &rhs) const { return !(operator==(rhs)); }

bool Localparam::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Localparam::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_value()) {
        cast_to<Localparam>(new_node)->set_value(get_value()->clone());
    }
    cast_to<Localparam>(new_node)->set_widths(Width::clone_list(get_widths()));
}
