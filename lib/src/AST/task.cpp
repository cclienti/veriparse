...
Task::Task(const Node::ListPtr ports, const Node::ListPtr statements, const std::string &name,
           const bool &automatic, const std::string &filename, uint32_t line)
    : Node(filename, line), m_ports(ports), m_statements(statements), m_name(name),
...
{
    cast_to<Task>(new_node)->set_ports(Node::clone_list(get_ports()));
