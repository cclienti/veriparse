...
PartselectPlusIndexed::PartselectPlusIndexed(const Node::Ptr index, const Node::Ptr size,
...
bool PartselectPlusIndexed::operator!=(const Node &rhs) const { return !(operator==(rhs)); }
...
{
    if(get_index()) {
        cast_to<PartselectPlusIndexed>(new_node)->set_index(get_index()->clone());
    }
    if(get_size()) {
        cast_to<PartselectPlusIndexed>(new_node)->set_size(get_size()->clone());
    }
    if(get_var()) {
        cast_to<PartselectPlusIndexed>(new_node)->set_var(get_var()->clone());
    }
}
