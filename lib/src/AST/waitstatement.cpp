...
WaitStatement::WaitStatement(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::WaitStatement);
    set_node_categories({NodeType::Node});
}

WaitStatement::WaitStatement(const Node::Ptr cond, const Node::Ptr statement,
...
bool WaitStatement::operator!=(const WaitStatement &rhs) const { return !(operator==(rhs)); }

bool WaitStatement::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool WaitStatement::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_cond()) {
        cast_to<WaitStatement>(new_node)->set_cond(get_cond()->clone());
    }
    if(get_statement()) {
        cast_to<WaitStatement>(new_node)->set_statement(get_statement()->clone());
    }
}
