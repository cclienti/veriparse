...
Width::Width(const std::string &filename, uint32_t line) : Node(filename, line)
{
...
bool Width::operator!=(const Width &rhs) const { return !(operator==(rhs)); }

bool Width::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Width::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    if(get_msb()) {
        cast_to<Width>(new_node)->set_msb(get_msb()->clone());
    }
    if(get_lsb()) {
        cast_to<Width>(new_node)->set_lsb(get_lsb()->clone());
    }
}
