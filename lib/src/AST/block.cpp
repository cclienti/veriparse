...
Block::Block(const std::string &filename, uint32_t line) : Node(filename, line)
{
    set_node_type(NodeType::Block);
    set_node_categories({NodeType::Node});
}

Block::Block(const Node::ListPtr statements, const std::string &scope, const std::string &filename,
             uint32_t line)
...
bool Block::operator!=(const Node &rhs) const { return !(operator==(rhs)); }

bool Block::remove(Node::Ptr node) { return replace(node, AST::Node::Ptr(nullptr)); }
...
{
    cast_to<Block>(new_node)->set_statements(Node::clone_list(get_statements()));
}
