...
std::string remove_leading_whitespace(const std::string &str)
{
    ...
    result.erase(result.begin(),
                 find_if_not(result.begin(), result.end(), [](int c) { return isspace(c); }));
    return result;
}
...
std::string remove_trailing_whitespace(const std::string &str)
{
    ...
    std::string result(str);
    result.erase(
        find_if_not(result.rbegin(), result.rend(), [](int c) { return isspace(c); }).base(),
        result.end());
    return result;
}
...
std::string remove_all_sub_string(const std::string &str, const std::string &sub_str)
{
    ...
    for(std::string::size_type i = result.find(sub_str); i != std::string::npos;
        i = result.find(sub_str)) {
        result.erase(i, n);
    }
