...
BranchSelection::BranchSelection(const FunctionMap &function_map) : m_function_map(function_map) {}
...
int BranchSelection::process(AST::Node::Ptr node, AST::Node::Ptr parent)
{
    ...
    if(node->is_node_type(AST::NodeType::IfStatement)) {
        ...
        const auto &cond_node =
            ExpressionEvaluation(m_function_map).evaluate_node(if_node->get_cond());
        ...
        auto comp_value =
            ExpressionEvaluation(m_function_map).evaluate_node(case_stmt_node->get_comp());
        ...
        if(comp_value) {
            ...
            for(const auto &cases_node : *case_stmt_node->get_caselist()) {
                ...
                for(const auto &cond : *conds) {
                    ...
                    const auto &equal_node = std::make_shared<AST::Eq>(
                        comp_value, cond_value, cases_node->get_filename(), cases_node->get_line());
