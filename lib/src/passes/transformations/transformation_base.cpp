...
{
    ProcessFunction process_function = [this](AST::Node::Ptr node, AST::Node::Ptr parent) -> int {
        return this->process(node, parent);
    };
...
int TransformationBase::recurse_in_childs(AST::Node::Ptr node, ProcessFunction function)
{
    int ret = 0;
    if(node) {
        ret = recurse(node, node->get_children(), function);
    }
    return ret;
}
