...
ParameterInliner::ParameterInliner() : m_paramlist(nullptr), m_paramlist_inst(nullptr) {}

ParameterInliner::ParameterInliner(AST::ParamArg::ListPtr paramlist_inst)
    : m_paramlist(nullptr), m_paramlist_inst(paramlist_inst)
{
}
        ...
        if(val) {
            ...
            if(m_paramlist_inst) {
                auto search = std::find_if(
                    std::begin(*m_paramlist_inst), std::end(*m_paramlist_inst),
            ...
            if(result != std::end(*m_paramlist)) {
                ...
                if(!value) {
                    ...
                    if(!value->is_node_type(AST::NodeType::Rvalue)) {
                        rvalue = std::make_shared<AST::Rvalue>(value, value->get_filename(),
                                                               value->get_line());
                    }
                    p->set_value(rvalue);
                }
