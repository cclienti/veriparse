...
int ModuleInstanceNormalizer::split_array(const AST::Node::Ptr &node, const AST::Node::Ptr &parent)
{
    ...
    if(instances->size() != 1) {
        LOG_ERROR_N(node)
            << "more than one element in the instance list, something failed during split";
        return 1;
    }
    ...
    for(std::int64_t i = array_dim.width - 1; i >= 0; i--) {
        ...
        for(auto &port : *portlist) {
            ...
            if(value_dims.is_fully_packed() &&
               arg_dims.packed_width() == value_dims.packed_width()) {
                continue;
            }
            ...
            if(width_div == 1) {
                ...
                std::size_t value_outer_index =
                    value_outer_is_big ? value_outer_msb - i : value_outer_msb + i;
                auto index_node = std::make_shared<AST::IntConstN>(
                    10, -1, true, Misc::Math::u64_to_mpz(value_outer_index));
                pnode = std::make_shared<AST::Pointer>(index_node, port->get_value());
            } else {
                // We need to slice the outer dimension because we will add
                // a partselect node.
                std::int64_t slice_msb = value_outer_is_big ? (value_outer_msb - width_div * i)
                                                            : (value_outer_msb + width_div * i);
                std::int64_t slice_lsb =
                ...
                auto slice_msb_node = std::make_shared<AST::IntConstN>(
                    10, -1, true, Misc::Math::i64_to_mpz(slice_msb));
                auto slice_lsb_node = std::make_shared<AST::IntConstN>(
                    10, -1, true, Misc::Math::i64_to_mpz(slice_lsb));
                pnode = std::make_shared<AST::Partselect>(slice_msb_node, slice_lsb_node,
                                                          port->get_value());
            }
...
int ModuleInstanceNormalizer::set_portarg_names(const AST::Node::Ptr &node,
                                                const AST::Node::Ptr &parent)
{
    ...
    if(!portlist) {
        ...
        for(const auto &argname : decl_portnames) {
            const auto &missing = std::make_shared<AST::PortArg>(
                nullptr, argname, instance->get_filename(), instance->get_line());
...
int ModuleInstanceNormalizer::fill_defparam_map(const AST::Node::Ptr &node,
                                                const AST::Node::Ptr &parent)
{
    if(!node) {
        return 0;
...
int ModuleInstanceNormalizer::set_paramarg_names(const AST::Node::Ptr &node,
                                                 const AST::Node::Ptr &parent)
{
    if(!node) {
        return 0;
    ...
    if(!paramlist) {
        ...
        for(const auto &param : *decl_paramlist) {
            ...
            auto new_param = std::make_shared<AST::ParamArg>(
                nullptr, param->get_name(), instance->get_filename(), instance->get_line());
        ...
        if(decl_paramlist->size() < paramlist->size()) {
            LOG_ERROR_N(module_decl)
                << "bad number of parameters for instance '" << instance->get_name() << "', "
        ...
        for(const auto &decl_param : *decl_paramlist) {
            const auto &missing = std::make_shared<AST::ParamArg>(
                nullptr, decl_param->get_name(), instance->get_filename(), instance->get_line());
...
int ModuleInstanceNormalizer::convert_to_lconcat(const AST::Node::Ptr &node,
                                                 const AST::Node::Ptr &parent)
{
    if(!node) {
        return 0;
    ...
    if(node->is_node_type(AST::NodeType::Concat)) {
        ...
        const auto &lconcat = std::make_shared<AST::Lconcat>(
            concat->get_list(), concat->get_filename(), concat->get_line());
...
int ModuleInstanceNormalizer::replace_port_affectation(const AST::Node::Ptr &node,
                                                       const AST::Node::Ptr &parent)
{
    if(!node) {
        return 0;
    ...
    for(const auto &port : *portlist) {
        ...
        auto it_out_search =
            std::find(module_decl_outputs.cbegin(), module_decl_outputs.cend(), port->get_name());
        if(it_out_search == module_decl_outputs.cend()) {
            rvalue =
                std::make_shared<AST::Rvalue>(port_value, port->get_filename(), port->get_line());
            lvalue =
                std::make_shared<AST::Lvalue>(identifier, port->get_filename(), port->get_line());
        } else {
            rvalue =
                std::make_shared<AST::Rvalue>(identifier, port->get_filename(), port->get_line());
            lvalue =
                std::make_shared<AST::Lvalue>(port_value, port->get_filename(), port->get_line());
        }
