...
inline void warn_fork_arm_label(const AST::Node::Ptr &arm)
{
...
inline bool contains_jump(const AST::Node::Ptr &node)
{
    if(!node) {
        return false;
    }
...
inline AST::BlockingSubstitution::Ptr make_ba(const std::string &target, const AST::Node::Ptr &rhs,
                                              const std::string &fn, int ln)
{
    auto lvalue = std::make_shared<AST::Lvalue>(fn, ln);
    lvalue->set_var(AST::to_node(make_id(target, fn, ln)));
...
inline bool contains_return(const AST::Node::Ptr &node)
{
    if(!node) {
        return false;
    }
...
inline int lower_return_list(const AST::Node::ListPtr &stmts, const std::string &task_name,
                             bool &always);
...
inline int lower_return_branch(AST::Node::Ptr &branch, const std::string &task_name, bool &always)
{
    always = false;
...
inline void must_writes(const AST::Node::Ptr &node, std::set<std::string> &writes);

inline void must_writes_list(const AST::Node::ListPtr &stmts, std::set<std::string> &writes)
{
    for(const auto &stmt : *stmts) {
