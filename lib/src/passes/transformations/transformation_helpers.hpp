...
template <typename T> inline std::set<T> to_set(const std::vector<T> &v)
{
    return std::set<T>(v.cbegin(), v.cend());
}

template <typename T> inline std::set<T> merge_set(const std::set<T> &s1, const std::set<T> &s2)
{
    std::set<T> new_set;
...
inline std::map<T_KEY, T_VAL> remove_keys(const std::map<T_KEY, T_VAL> &map,
                                          const std::set<T_KEY> &keys)
{
