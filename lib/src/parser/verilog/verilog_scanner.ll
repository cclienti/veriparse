...
"ref"         {
                  if(m_sv_mode) return token::TK_REF;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"always_comb" {
...
"void"        {
                  if(m_sv_mode) return token::TK_VOID;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"return"      {
...
"enum"        {
                  if(m_sv_mode) return token::TK_ENUM;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"struct"      {
...
"packed"      {
                  if(m_sv_mode) return token::TK_PACKED;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"package"     {
...
"export"      {
                  if(m_sv_mode) return token::TK_EXPORT;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"static"      {
...
"interface"   {
                  if(m_sv_mode) return token::TK_INTERFACE;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"endinterface" {
                  if(m_sv_mode) return token::TK_ENDINTERFACE;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"modport"     {
                  if(m_sv_mode) return token::TK_MODPORT;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
"virtual"     {
                  if(m_sv_mode) return token::TK_VIRTUAL;
                  yylval->build<std::string>(yytext);
                  return token::TK_IDENTIFIER;
              }
