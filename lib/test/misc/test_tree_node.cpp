...
public:
    ...
    static void reset() { TNode::set_tree_node_id(0); }

private:
    std::string print_value() const final { return get_value(); }

    Ptr make_ptr(const std::string &value) const final { return std::make_unique<TNode>(value); }
};
