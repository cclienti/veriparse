...
#define TEST_CORE                                                                                  \
    ...
    test_helpers.render_node_to_dot_file(source, test_string + ".dot");                            \
                                                                                                   \
    /* load the reference */                                                                       \
    std::string test_ref_suffix = "refs/deadcode_elimination_";                                    \
...
// A hierarchical write is not this module's to remove: `bus.sum` survives
// while the dead local of the same leaf name goes.
TEST(PassesTransformation_Deadcode, sv_deadcode_hier0) { TEST_CORE_SV; }
